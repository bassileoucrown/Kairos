...
(async () => {
  ...
  try {
    ...
    const OPENERS = [
      ...
      // Each of these now begins by PICKING a message: the verbs belong to the
      // one somebody tapped rather than sitting under every line.
      [`/threads/${threadId}`, 'the edit box',
        ['.msg-note:has-text("Understood")', '.msg-note:has-text("Understood") button:has-text("Edit")'], '.msg-edit'],
      [`/threads/${threadId}`, 'the record picker',
        ['.msg-note:has-text("Understood")', 'button:has-text("Promote to record")'], '.msg-promote-picker'],
      [`/threads/${threadId}`, 'the task maker',
        ['.msg-note:has-text("Understood")', 'button:has-text("Make a task")'], '.msg-task-form'],
      [`/threads/${threadId}`, 'the actions on a picked message',
        ['.msg-note:has-text("Understood")'], '.msg-actions-row'],
      [`/threads/${threadId}`, 'superseding a record',
        ['.msg-record', 'button:has-text("Supersede")'], '.msg-supersede'],
