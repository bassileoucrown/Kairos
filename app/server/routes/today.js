...
router.get('/:ownerId', requirePaAccess, (req, res) => {
  ...
  const schedule = buildDay(req.principal, todayKey);
  ...
  const visibleSpaceIds = listVisibleSpaces(req.user.id).map((s) => s.id);
  let recordsAwaiting = [];
  if (visibleSpaceIds.length > 0) {
    const placeholders = visibleSpaceIds.map(() => '?').join(',');
    recordsAwaiting = db.prepare(`
